...
const StaticNoise = () => {
    ...
    useEffect(() => {
        ...
        const draw = () => {
            ...
            for (let i = 0; i < len; i++) buffer32[i] = Math.random() < 0.5 ? 0xff000000 : 0xffffffff;
    ...
    return <canvas ref={canvasRef} className="fixed inset-0 pointer-events-none z-[9999] mix-blend-overlay opacity-[0.08]" />;
};

const AppCard = ({ title, logo, onClick }) => (
    <motion.div whileHover={{ scale: 1.05, y: -5 }} whileTap={{ scale: 0.95 }} className="relative group cursor-pointer flex flex-col items-center" onClick={onClick}>
...
export default function OnoraIntro() {
    ...
    useEffect(() => { const timer = setTimeout(() => setIsScreenOn(true), 1000); return () => clearTimeout(timer); }, []);
    return (
        <div className="relative w-full h-full bg-[#050505] overflow-hidden font-sans text-white flex items-center justify-center">
            <StaticNoise />
            <motion.div
                initial={{ scaleY: 0.005, scaleX: 0, opacity: 0 }}
                animate={{ scaleY: isScreenOn ? 1 : 0.005, scaleX: isScreenOn ? 1 : (isScreenOn === false ? 0 : 1), opacity: 1 }}
                transition={{ duration: 0.8, ease: "easeInOut", times: [0, 0.4, 1] }}
                className="relative w-full h-full flex flex-col"
                style={{ boxShadow: "inset 0 0 100px rgba(0,0,0,0.9)" }}
            >
                <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: isScreenOn ? 1 : 0, y: isScreenOn ? 0 : -20 }} transition={{ delay: 1.5, duration: 1 }} className="absolute top-0 left-0 w-full p-6 md:p-10 flex justify-between items-center z-50">
                    <img src={ASSETS.mainLogo} alt="ONORA" className="h-12 md:h-16 object-contain cursor-pointer hover:opacity-80 transition-opacity" onClick={() => window.location.reload()} />
                    <button className="border border-white/20 bg-black/50 backdrop-blur-md px-6 py-2 rounded-full text-[10px] md:text-xs uppercase tracking-[0.2em] hover:bg-white/10 transition-all">Access Waitlist</button>
                </motion.div>
                <div className="absolute inset-0 z-0">
                    <motion.video initial={{ opacity: 0 }} animate={{ opacity: isScreenOn ? 0.6 : 0 }} transition={{ delay: 1, duration: 2 }} src={ASSETS.robotVideo} autoPlay muted loop playsInline className="w-full h-full object-cover grayscale-[30%] contrast-125" />
                ...
                <div className="relative z-20 flex-grow flex flex-col items-center justify-center pt-20">
                    <motion.div initial={{ opacity: 0, scale: 0.9 }} animate={{ opacity: isScreenOn ? 1 : 0, scale: isScreenOn ? 1 : 0.9 }} transition={{ delay: 2.5, duration: 1 }} className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-12">
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 4, duration: 1 }} className="absolute bottom-10 flex flex-col items-center gap-2 opacity-50 animate-bounce">
